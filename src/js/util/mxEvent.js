...
=>
{
  ...
  updateListener = (element, eventName, funct) => {
    ...
    {
      var
      listenerCount = element.mxListenerList.length;
      ...
      {
        var
        entry = element.mxListenerList[i];
...
(element) => {
  ...
  if (list != null) {
    ...
    addGestureListeners
:
(node, startListener, moveListener, endListener) => {
...
(node, startListener, moveListener, endListener) => {
  ...
  if (!mxClient.IS_POINTER && mxClient.IS_TOUCH) {
    ...
    redirectMouseEvents
:
(node, graph, state, down, move, up, dblClick) => {
  var getState = (evt) => {
